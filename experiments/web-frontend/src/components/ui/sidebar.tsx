...
import { IconArrowLeft, IconMessageBolt } from "@tabler/icons-react";
...
>(
  (
    ...
    const defaultOpen = localStorage.getItem(SIDEBAR_COOKIE_NAME) === "true";
...
>(
  (
    ...
    const { isMobile, state, open, toggleSidebar } = useSidebar();
    ...
    if (isMobile) {
      return (
        <Sheet open={open} onOpenChange={toggleSidebar} {...props}>
          <SheetContent
    ...
    return (
      ...
      >
        ...
        >
          <div
            data-sidebar='sidebar'
            className='overflow-y-auto scrollbar-thumb-gray-700 flex h-full w-full flex-col bg-sidebar group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:border group-data-[variant=floating]:border-sidebar-border group-data-[variant=floating]:shadow'
          >
            <div className='right-0 absolute z-10 pt-2 pr-5'>
              <SidebarMinimize />
            </div>
            {children}
          </div>
...
const SidebarMinimize = forwardRef<
  ElementRef<typeof Button>,
  ComponentProps<typeof Button>
>(({ className, onClick, ...props }, ref) => {
  const { toggleSidebar } = useSidebar();
  ...
  return (
    <Button
      ...
      {...props}
    >
      <IconArrowLeft className='' style={{ width: "35px", height: "35px" }} />

      <span className='sr-only'>Toggle Sidebar</span>
    </Button>
  );
});
SidebarMinimize.displayName = "SidebarMinimize";
